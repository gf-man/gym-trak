    ...
    for ex in exercise_list:
        if record_exercise == ex.name:
            if ex.category == Exercise_Type["Resistance"]:
                ...
                dist = input("Distance(kilometers): ")
...
def view_todays_records():
    for ex in exercise_list:
        if get_date() in ex.record:
            print(ex.name, "x".join(ex.record[get_date()]))
    ...
    for ex in exercise_list:
         if ex_name == ex.name:
            for record in ex.record:
                print(record, "x".join(ex.record[record]))
    ...
    for rec_date in date_list:
        ...
        for ex in exercise_list:
            if rec_date in ex.record:
                print(ex.name, "x".join(ex.record[rec_date]))
        print()
...
if __name__ == "__main__":
    ...
    load_records()
    while True:
        print("N: New Exercise, R: Add Record, V: View Today's Record, A: View All Records, E: View Exercise Records, S: Save Records")
        option = input()
        if option.lower() == "n":
            input_exercise()
        elif option.lower() == "r":
